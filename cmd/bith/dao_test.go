...
func testDAOForkBlockNewChain(t *testing.T, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	...
	// Start a bith instance with the requested flags set and immediately terminate
	if genesis != "" {
		...
		bith := runGeth(t, append(args, []string{"--exec", "2+2", "console"}...)...)
		bith.WaitExit()
	}
	// Retrieve the DAO config flag from the database
	path := filepath.Join(datadir, "bith", "chaindata")
