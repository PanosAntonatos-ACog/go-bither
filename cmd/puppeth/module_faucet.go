...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/bitherhq                                                            && \
	(cd $GOPATH/src/github.com/bitherhq && git clone --depth=1 https://github.com/bitherhq/go-bither) && \
  go build -v github.com/bitherhq/go-bither/cmd/faucet                                              && \
